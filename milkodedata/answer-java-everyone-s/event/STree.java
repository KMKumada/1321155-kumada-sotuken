...
//      sliders.setPreferredSize(new Dimension(150,20));
      ...
      cpanel.add(new JLabel("‰ñ”:"));
      cpanel.add(ntf);
      cpanel.add(new JLabel("  }‚ÌL‚ª‚è:"));
      cpanel.add(slidera);
      cpanel.add(new JLabel("  }‚Ì’·‚³:"));
      cpanel.add(sliders);
   ...
   void drawTree(double x1, double y1, double len, double stand, int nn){
      double x2,y2;
      x2=x1 + len*Math.sin(Math.toRadians(stand));  //`}æ‚ÌxÀ•W`
      y2=y1 - len*Math.cos(Math.toRadians(stand));  //`}æ‚ÌyÀ•W`
      bg.drawLine((int)x1,(int)y1,(int)x2,(int)y2); //`ü‚ğ•`‚­`
      if(nn>1){   //`–Ø‚Ì––’[‚Ü‚Å’B‚µ‚Ä‚¢‚È‚¢‚ÍCŸ‚Ì}‚ğ•`‚­`
	 drawTree(x2,y2,len*scale,stand-angle,nn-1); //`Ÿ‚Ì‰E‚Ì}‚ğ•`‚­`
	 drawTree(x2,y2,len*scale,stand+angle,nn-1); //`Ÿ‚Ì¶‚Ì}‚ğ•`‚­`
      }
   }
   void drawing(){
      ...
      drawTree(W/2, H-50, len, 0, n);  // `Ä‹Aˆ—‚ÌŠJn`
   }
